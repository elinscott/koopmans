...
class TestPoolsDivideRanks:
    ...
    def test_computer_default_source_is_named(
        ...
        computer = installed_pw_code.computer
        previous = computer.get_default_mpiprocs_per_machine()
        computer.set_default_mpiprocs_per_machine(14)
        try:
            inp = KoopmansInput.model_validate(_pw_input(parallelization={"pw": {"npool": 4}}))
            with pytest.raises(ValueError) as excinfo:
                build_workgraph(inp)
        finally:
            computer.set_default_mpiprocs_per_machine(previous)
    ...
    def test_code_without_pool_support_is_left_alone(self) -> None:
        ...
        check_pools_divide_ranks({"wannier90": {"ntasks": 14, "npool": 4}}, {"wannier90": object()})
