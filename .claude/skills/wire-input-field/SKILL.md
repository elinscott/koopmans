...
3. **Locate the new wiring path**:
   - **Global flow:** `KoopmansInput` → `koopmans2/src/koopmans/aiida/workflows/__init__.py::prepare_common_inputs` → `koopmans2/src/koopmans/aiida/conversion.py::input_to_pw_parameters` (or equivalent for other calculators) → `overrides` dict → `get_builder_from_protocol` → builder → `get_dict_from_builder` → `@task.graph` task inputs.
...
5. **Write the wiring.** Minimal edit in `koopmans2/src/koopmans/aiida/conversion.py` (or `prepare_common_inputs` in the `workflows` package) to inject the field into the right `overrides` key. Follow the existing shape exactly — do not refactor the converter structure.
