...
6. **Update the dispatcher.** New tasks need a `Task` enum value (`koopmans2/src/koopmans/input_file/workflow.py`), a codes TypedDict in `aiida-koopmans2/workgraphs/codes.py` loaded via `load_codes`, and a `build_<task>_workgraph` route module under `aiida/workflows/` wired into `build_workgraph`.
