...
- Dispatcher pattern (`aiida/workflows/`): one `build_<task>_workgraph` route module per `Task` enum value + code loading via `load_codes_for_task`; the package `__init__` dispatches and attaches input-file advice to plugin errors. Keep dispatch thin.
