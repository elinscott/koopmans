...
- Dispatcher pattern (`aiida/workflows/`): one `build_<task>_workgraph` route module per `Task` enum value + per-chain code loading via `load_codes(<Chain>Codes)` inside the route; the package `__init__` dispatches and attaches input-file advice to plugin errors. Keep dispatch thin.
