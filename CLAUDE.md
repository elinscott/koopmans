...
2. **No duplication.** Before writing any helper, search `variational_orbitals.py`, `projections.py`, `occupations.py`, `workgraphs/utils/wannier_merge.py`, `workgraphs/ml/helpers.py`, `spin.py`, `ml.py`, `functionals.py`, `screening.py`, `parallelization.py` (ak2) and `conversion.py` (k2); extend in place. New modules only for genuinely new orchestration. Same rule for tests: shared fixtures and builders live in `tests/fixtures.py` (re-exported via `conftest.py`) — never define a fixture module-locally that a sibling module already has or could share.
   The rule extends to dependencies: **search the libraries we already stand on before hand-rolling**. A hand-rolled replica of machinery a dependency owns re-implements its edge cases and drifts from them silently (a reciprocal-cell matrix aiida-core's `KpointsData.reciprocal_cell` already computes; an argv scanner replicating click's tokenizer to learn option positions its parser already tracks). Check the obvious owners first — aiida-core, click, ASE, numpy, the QE plugins — and reach for their surface before writing a parallel implementation. A public surface needs no test of ours — upstream tests it. A semi-private one earns exactly one canary test that pins the private name still exists and answers as we rely on it; never tests of upstream behavior beyond that.
...
5. **Explicit failure over silent ignore.** An input that cannot take effect raises `NotImplementedError`/`ValueError` naming the gap — no keyword is silently dropped. A block that mirrors an external namelist rejects every keyword the workflow itself computes, derives, or hardcodes, so that anything a user is left free to set is a value that actually reaches the calculation.
...
- Dispatcher (`aiida/workflows/`): covers `DFT_BANDS`, `WANNIERIZE`, `SINGLEPOINT` (DSCF via kcp.x with KI/KIPZ, molecular KS-init and periodic Wannier-init routes; DFPT via kcw.x, whose interpolated band structure takes `kpoints.smooth_interpolation_factor` too), `TRAJECTORY` (ML via `ml: {mode: train|test|predict}`, `self_hartree` descriptor; predict swaps each snapshot's Delta-SCF refinement for a trained-model prediction off the trial KI's self-Hartrees, feeding the final KI via the `initial_alphas`-style injection; test additionally runs a second final KI at model-predicted alphas per snapshot and reports the per-orbital alpha deltas and final-KI eigenvalue max/RMS under the evaluation's `alpha_and_eigenvalue_deltas`; multi-snapshot via `atoms.snapshots`, a multi-frame xyz mutually exclusive with explicit `atomic_positions`, resolved against the input file's directory), `DFT_EPS` (ph.x dielectric), and `BSE` (Bethe-Salpeter on Koopmans QP energies via yambo, composed onto the DFPT singlepoint; `calculator_parameters.yambo`; needs `k2y`/`aiida-yambo` alongside the yambo executables). Live-validated on silicon 2026-09-17 (yambo 5.3, k2y QP database from kcw.x eigenvalues, `parallelization.yambo.bethe_salpeter` split).
...
- Periodic DSCF (mlwfs/projwfs): wannierize → fold-to-supercell (wann2kcp.x + merge_evc.x) → Wannier-seeded kcp.x init; supercell-image orbital grouping approximated via the defaulted `group_orbitals_by='self_hartree'` / `group_orbitals_tol=1e-4` (constructive grouping not ported). Fold path live-validated on the silicon tutorial (2026-09-11): two screening groups, final KI HOMO/LUMO within 0.3 meV of legacy, smooth-interpolated gap 1.1814 eV vs legacy 1.1816 through the same interpolator. A `kpoints.path` also reaches every Wannierization the route runs (the initialization one always, plus the denser smooth-interpolation mesh when `smooth_interpolation_factor` asks for one), so each publishes its own pw.x explicit and wannier90-interpolated base-functional bands off its own dumped steps — not a named workflow output; `koopmans plot bandstructure` finds them by provenance, disambiguated by the wannierize/wannierize_smooth step name when both ran.
...
- aiida-core: builds from `../aiida-core`; aiida-shell rides its git master via `[tool.uv.sources]` until a post-`Code.Model` release exists. CI/RTD clone pin: `elinscott/aiida-core` @ `715972d65e` (branch `dump-data-json`, upstream PR aiidateam/aiida-core#7589: the `include_data_json`/`include_workflow_outputs` dump flags; upstream main as of 2026-08-27 — 92c21cc2dd plus three changelog commits and the stash-failure logging fix #7572 — with those commits on top). Move the pin back to upstream once #7589 merges. NOTE: `uv run --project` re-syncs the venv whenever `../aiida-core` changes — keep that checkout where the CI pin points.
...
- Known gaps (raise `NotImplementedError` with pointers in `aiida/workflows/`): corrections `PKIPZ`/`NONE`/`ALL`; `init_orbitals='pz'`; `fix_spin_contamination`; gamma-only/molecular DFPT; `eps_inf='auto'` for DSCF (wired for DFPT); `convergence` task. `eps_inf='auto'` on the DFPT singlepoint runs its dielectric scf on `kpoints.grid` unless `kpoints.eps_inf_factor` (a per-direction densification factor, modelled on `smooth_interpolation_factor`; default `(1, 1, 1)`) densifies it — eps_inf converges much more slowly with k-points than the singlepoint itself; the base is always `kpoints.grid`, never `overrides.scf`, and the mesh's offset follows the chain's own scf. The default factor prints a build-time advisory naming the mesh used. `bse` composes the same DFPT chain but does not yet thread `eps_inf_factor` through it (refused at parse); `task: dft_eps` already samples its own dielectric scf via `kpoints.grid` and refuses `eps_inf_factor` too.
- Standalone UI (unfold-and-interpolate) is unsupported by decision: `task: unfold_and_interpolate` is rejected at parse (no `Task` enum member). The singlepoint-embedded interpolation lives under `kpoints.smooth_interpolation_factor` (replaces the former `calculator_parameters.unfold_and_interpolate` block, which the parser now refuses outright); `use_ws_distance`/`do_dos` are no longer user-facing and run at aiida-koopmans's defaults (both `True`). Both singlepoint routes take the factor: the ΔSCF route interpolates the kcp.x supercell Hamiltonian, the DFPT route kcw.x's `*.kcw_hr_{occ,emp}.dat` (`spin` `none`/`collinear` only; noncollinear raises), through the one shared interpolator in `aiida-koopmans2/workgraphs/ui/`. With the factor set the route publishes the smooth band structure as its own; kcw.x's coarse interpolation still runs and stays addressable on the ham step.
- Parallelization: a top-level per-code `parallelization` block (`input_file/parallelization.py`) sets each code's MPI ranks (`ntasks` → `metadata.options.resources`), k-point pools (`npool` → `-npool`), and pencil decomposition (`pd` → `-pd true`). Per-code flag support follows legacy `commands.py`: npool for pw/projwfc/kcw (kcw only on its wann2kc + screen steps, not ham), pd for pw/pw2wannier90/projwfc/kcw; the schema rejects unsupported flag/code combos. The dispatcher threads the whole mapping to every graph builder; the pw.x steps also ride the shared overrides. The legacy `workflow.npool` shorthand has been removed. `parallelization.yambo` also takes per-driver MPI role splits (`bethe_salpeter` / `static_screening` / `dipoles`, role → rank count, product = `ntasks`), translated to yambo's `*_CPU`/`*_ROLEs` by aiida-koopmans.
- Remote computers: the top-level `computer:` block (`name`, `walltime` as an ISO 8601 duration, `account`, `queue`) targets any configured AiiDA computer, with codes registered via `verdi code create` for computers outside `koopmans install`'s roster (see `src/koopmans/input_file/computer.py` for the exact field names).
- Branch state: everything above is merged to `main` in both repos. PR-per-change flow via the `pr` remotes; branches, commits, and PRs on the elinscott-owned repos are authored by the `elinsc-bot` machine account; merges are the user's.
