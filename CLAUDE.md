...
- aiida-core: builds from `../aiida-core`; aiida-shell rides its git master via `[tool.uv.sources]` until a post-`Code.Model` release exists. CI/RTD clone pin: `elinscott/aiida-core` @ `715972d65e` (branch `dump-data-json`, upstream PR aiidateam/aiida-core#7589: the `include_data_json`/`include_workflow_outputs` dump flags; upstream main 92c21cc2dd plus those commits). Move the pin back to upstream once #7589 merges. NOTE: `uv run --project` re-syncs the venv whenever `../aiida-core` changes — keep that checkout where the CI pin points.
