...
- Known gaps (raise `NotImplementedError` with pointers in `aiida/workflows/`): corrections `PKIPZ`/`NONE`/`ALL`; `init_orbitals='pz'`; `fix_spin_contamination`; gamma-only/molecular DFPT; `eps_inf='auto'` for DSCF (wired for DFPT); `convergence` task. `eps_inf='auto'` on the DFPT singlepoint runs its dielectric scf on the chain's own `scf` mesh unless `kpoints.overrides.ph` (a `StepKpointsOverridesInput`, same as `scf`/`nscf`) gives it a mesh or spacing of its own — eps_inf converges much more slowly with k-points than the singlepoint itself; an unset `overrides.ph` prints a build-time advisory naming the fallback mesh. `bse` composes the same DFPT chain but does not yet thread `overrides.ph` through it (refused at parse); `task: dft_eps` already samples its own dielectric scf via `overrides.scf` and refuses `overrides.ph` too.
