...
- Periodic DSCF (mlwfs/projwfs): wannierize → fold-to-supercell (wann2kcp.x + merge_evc.x) → Wannier-seeded kcp.x init; supercell-image orbital grouping approximated via the defaulted `group_orbitals_by='self_hartree'` / `group_orbitals_tol=1e-4` (constructive grouping not ported). Fold path live-validated on the silicon tutorial (2026-09-11): two screening groups, final KI HOMO/LUMO within 0.3 meV of legacy, smooth-interpolated gap 1.1814 eV vs legacy 1.1816 through the same interpolator. A `kpoints.path` also carries the base-functional band structure (`KoopmansDSCFOutputs.band_structure_dft`, ak2), off the pw.x quality-check run of whichever Wannierization actually samples the path — the smooth-mesh one when `smooth_interpolation_factor` asks for one, the initialization one otherwise, on the same absolute energy scale as the KI bands.
