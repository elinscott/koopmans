...
- Periodic DSCF (mlwfs/projwfs): wannierize → fold-to-supercell (wann2kcp.x + merge_evc.x) → Wannier-seeded kcp.x init; supercell-image orbital grouping approximated via a defaulted `orbital_groups_self_hartree_tol=1e-4` (constructive grouping not ported). **Live QE smoke test passes end-to-end** (2026-07-07, Si 2×2×2 KI, root Finished [0] incl. the PW-vs-CP init consistency check; fixes spanned all 5 repos — see the uncommitted wave-2 working tree + `koopmans backend daemon`/HQ-worker memory notes). Remaining physics follow-up: screening-parameter magnitudes and the final KI spectrum diverge from legacy (suspect `do_ee`/`&EE` + innerloop namelist deltas vs legacy `dft_init`; verify via koopmans-vs-koopmans2 regression against the fresh legacy tutorial-2 run in `../koopmans/tutorials/tutorial_2/01-koopmans-dscf`, same GNU binaries). Wannierization scf/nscf now shared from `Wannier90WorkChain.get_scf_nscf_builders_from_protocol` (added to the aiida-wannier90-workflows fork); block-level overrides are the flat `WannierizeOverrides` TypedDict (`w90_keywords`/`pw2wannier90_keywords`), never the upstream nested shape.
