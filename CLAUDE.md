...
- Periodic DSCF (mlwfs/projwfs): wannierize → fold-to-supercell (wann2kcp.x + merge_evc.x) → Wannier-seeded kcp.x init; supercell-image orbital grouping approximated via a defaulted `orbital_groups_self_hartree_tol=1e-4` (constructive grouping not ported). **Live QE runs reach exact numerical parity with legacy** (2026-07-08, Si 2×2×2 KI vs the legacy tutorial-2 run with the same GNU binaries: α = 0.1330/0.0404 identical, trial λ matching to all digits, KI gap 1.2790 vs 1.2792 eV). Parity required, beyond the env/plumbing fixes: legacy's disentanglement-pool rule for the uppermost block, `nr1b/2b/3b` autogeneration for core-corrected pseudos, fractional-site (bond-centred) projections, and `do_ee` always on with `EE.which_compensation` 'none'/'tcc' (EE participates in the orbital SIC even uncompensated). Still unexercised live: `ham_*.dat` band-path retrieve globs (`calculate_bands`); the parity input should become a permanent regression test. Wannierization scf/nscf now shared from `Wannier90WorkChain.get_scf_nscf_builders_from_protocol` (added to the aiida-wannier90-workflows fork); block-level overrides are the flat `WannierizeOverrides` TypedDict (`w90_keywords`/`pw2wannier90_keywords`), never the upstream nested shape.
...
- ML descriptors: `self_hartree`, and `orbital_density` via the user's wannier90 `feature/decompose` fork (`wannier_decompose` keywords through the wannier overrides; per-WF `.power` files harvested from the trial-KI wannierization's `wannier_blocks` output — requires the decompose-enabled wannier90.x binary at runtime). `orbital_density` works for `ml:train`/`ml:test`; predict+orbital_density is still gated (the predict path must forward the wannier artefacts).
- Periodic DSCF band structures: `calculate_bands` + `kpoints.path` runs per-(filling, spin) unfold-and-interpolate off the final KI's printed `ham_*.dat` + the init wannierization's hr files, incl. smooth interpolation (`smooth_int_factor` reruns the wannierization on the denser grid).
- Known gaps (raise `NotImplementedError` with pointers in `aiida/workflows.py`): corrections `PKIPZ`/`NONE`/`ALL`; `init_orbitals='pz'`; `fix_spin_contamination`; gamma-only/molecular DFPT; `eps_inf='auto'` for DSCF (wired for DFPT); predict+orbital_density; `convergence` task; constructive supercell-image orbital grouping (1e-4 self-Hartree stopgap in place).
- Branches (PR-per-change flow, user pushes/opens PRs), review/merge order: `core_functionality` → `spin-dfpt` → siblings {`dft-eps`, `ui-task`, `dscf-mlwf-init`} → `dfpt-multiblock` (on dft-eps), `ml-completion` (on dscf-mlwf-init) → `ui-singlepoint` (merge of ml-completion + ui-task + dfpt-multiblock, + one commit) → `w90-descriptors` (on ui-singlepoint); `wave-2` = working superset, content-identical to `w90-descriptors`.
