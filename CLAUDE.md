...
- aiida-pythonjob: `[tool.uv.sources]` now points `../aiida-pythonjob` at the fork's `patched` branch (tip `a9dbf86`, upstream main `fb8eb96` plus the `keep-declared-nodes` commits: a leaf whose `input_model` declares `orm.StructureData` receives the node itself, not its unwrapped value). CI and RTD clone `elinscott/aiida-pythonjob` at `patched`, mirroring `aiida-workgraph`/`node-graph`. The direct `aiida-pythonjob~=0.5.2` dependency (see its inline comment) still honors this override the same way `node-graph` does.
