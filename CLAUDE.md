...
- aiida-core: builds from `../aiida-core` at current upstream main (post-v2.8, includes the workgraph-dump MRO fix); aiida-shell rides its git master via `[tool.uv.sources]` until a post-`Code.Model` release exists. CI/RTD clone pin: `92c21cc2d9` (includes #7525, the process-dump `node_outputs` gating fix — the local `fix/dump-arraydata-outputs` checkout divergence is resolved now that this pin carries it). NOTE: `uv run --project` re-syncs the venv whenever `../aiida-core` changes — keep that checkout where the CI pin points.
