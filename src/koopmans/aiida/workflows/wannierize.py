...
def build_wannierize_workgraph(koopmans_input: KoopmansInput) -> WorkGraph:
    ...
    * Explicit projections or ``block_wannierization_threshold`` route to
    ...
    # By-need loading: the wannierize bodies subscript codes at build time,
    # so their needed members must exist before the build (node-graph#169).
...
def _build_wannierize_blocks_workgraph(koopmans_input: KoopmansInput) -> WorkGraph:
    ...
    # graph decides whether the projected DOS runs. By-need loading: the
    # wannierize bodies subscript codes at build time, so their needed
    # members must exist before the build (node-graph#169). Requiring
    # ``__optional_keys__`` outright would also demand projwfc, which is
    # not part of the split machinery, so the requirement stays the
    # explicit ``("wannierjl",)`` tuple.
