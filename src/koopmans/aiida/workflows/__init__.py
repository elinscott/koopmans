...
def _declared_code_help(member: str) -> str | None:
    ...
    conflicting ones, returns ``None`` — a socket path's task component
    is a call-site label, so it cannot say which chain's purpose applies.
    """
...
def _missing_inputs_advice(exc: MissingRequiredInputsError) -> str | None:
    ...
    purpose the chain TypedDicts agree on (:func:`_declared_code_help`).
    Entries of other socket types are not code-installation problems, so
    an error naming only those earns no advice.
    """
    missing: list[tuple[str, str | None]] = []
