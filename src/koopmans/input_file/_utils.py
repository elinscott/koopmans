...
def reject_route_owned_fields(model: BaseModel, owners: dict[str, str], block: str) -> None:
    ...
    pattern koopmans itself uses to re-validate a modified input. A field
    the caller always forces to a value other than its inherited default
    should redeclare that default on the subclass, so "the field's own
    declared default" and "what actually runs" agree.
