...
from wannier90_input.models.parameters import CentreConstraint, DisentanglementSphere, FractionalCoordinate, NearestNeighborKpoint, Projection, SpecialPoint
...
_WANNIER90_OWNED: dict[str, str] = {
    ...
    'exclude_bands': 'Which bands each block spans follows from `calculator_parameters.wannier90.projections`.',
    ...
    'spin': 'Set `workflow.spin`.',
    'spinors': 'Set `workflow.spin`.',
    'unit_cell_cart': 'The cell comes from the `cell_parameters` block.',
    'write_hr': 'koopmans always asks wannier90 for the gauge products; the screening and the supercell fold read them.',
    'write_u_matrices': 'koopmans always asks wannier90 for the gauge products; the screening and the supercell fold read them.',
    'write_xyz': 'koopmans always asks wannier90 for the gauge products; the screening and the supercell fold read them.',
}


class _Wannier90Input(Wannier90InputTemplate):
