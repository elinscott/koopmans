...
from koopmans.input_file.cell_parameters import (CellParametersViaAlat,
...
class CalculatorParametersInput(BaseModel):
    ...
    pw2wannier90: PW2Wannier90InputParameters = Field(default_factory=lambda: PW2Wannier90InputParameters())
    wannier90: Wannier90InputParametersWithUpDown = Field(default_factory=lambda: Wannier90InputParametersWithUpDown())
...
class KoopmansInput(BaseModel):
    ...
    workflow: WorkflowConfig = Field(description="Configuration specifying the workflow to be executed")
    atoms: AtomsInput = Field(description="Atomic structure information")
    kpoints: KpointsInput = Field(default_factory=lambda: KpointsInput(gamma_only=True), description="k-point sampling information")
    calculator_parameters: CalculatorParametersInput = Field(description="Parameters for the individual electronic structure calculators (``pw.x``, etc...)")
